...
class Cv3GExport implements FromQuery, WithHeadings, ShouldAutoSize, WithStyles
{
    use Exportable;
    ...
    {
        $head1 = ['id','cc','region','departamento','provincia','distrito','ccpp'];
        $head2 = ['multa_umts','periodo_multa','fecha_comentario_mejora','comentario_mejora','fecha_comentario_diagnostico','comentario_diagnostico','fecha_solucion','sustento_solucion','fecha_dt','valor_dt','ruta_mediciones','fecha_insercion','ccpp_observacion_3g','responsable_principal','analista_responsable','diagnostico','fecha_diagnostico','accion_realizada','forecast_principal','solucion_principal','estado','comentarios'];
        $periodos = DB::table('prg_cv_periodos')->where('estado',1)->orderBy('id','asc')->pluck('periodo')->toArray();    
        $headf = array_merge($head1,$periodos,$head2);
        return CV_3G::select($headf);
    }
