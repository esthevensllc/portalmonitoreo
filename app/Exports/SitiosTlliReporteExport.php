...
use App\Models\SITIOS_OBSERVADOS_TLLI;
...
class SitiosTlliReporteExport implements FromQuery, WithHeadings, WithStyles, WithColumnWidths,ShouldAutoSize
{
    use Exportable;
    ...
    {
        return SITIOS_OBSERVADOS_TLLI::select('site_name','bcf_address','departamento','tipo_obs_fitel','tecnologia','mes','anio')->where('estado',1)->where('mes',$this->p_mes)->where('anio',$this->p_anio);
    }

    public function styles(Worksheet $sheet)
    {
        
        $motivos = DB::select("SELECT site_name,bcf_address,departamento,tipo_obs_fitel,tecnologia,mes,anio FROM PRG_SITIOS_OBSERVADOS_TLLI WHERE mes = $this->p_mes AND anio = $this->p_anio");
