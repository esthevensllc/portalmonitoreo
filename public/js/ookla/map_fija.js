class MapController{
    ...
    attachHandlers(){
        ...
        let renderLayerHandler = function(){
            ...
            fetch(`${self.config.base_url}/ookla-map-fija/kpis/${kpiMonthsSelector.value}/${kpiOperatorSelector.value}/${kpiLayerSelector.value}`, {
                headers: {
            ...
            .then(response => {
                ...
                if(response.data){
                    ...
                    let kpiConfig = {
                        ...
                        'val_download_kbps': {
                            colorDomain: [15, 30, 45, 90, 150, 300],
                            colorRange: ['#950F0F','#FF0000', '#FF8700', '#FFD300', '#0aff33ff', '#1d7c35ff'],
                        },
                        'val_upload_kbps': {
                            colorDomain: [7.5, 15, 23, 45, 75, 150],
                            colorRange: ['#950F0F','#FF0000', '#FF8700', '#FFD300', '#0aff33ff', '#1d7c35ff'],
                        },
    ...
    onReady(){
        let self = this;
        /*
        fetch(`${this.config.base_url}/ookla-map-fija/kpis/months`, {
            headers: {'Accept': 'application/json'}
        })
        .then(response => response.json())
        .then(response => {
        ...
        */
        ...
        document.querySelector('#check_layer_planos').dispatchEvent(new Event('change'));
        document.querySelector('#kpi_layers').dispatchEvent(new Event('change'));

        fetch(`${this.config.base_url}/ookla-map-fija/kpis/last-months`, {
            headers: {'Accept': 'application/json'}
        })
        .then(response => response.json())
        .then(response => {
            ...
            response.data = [];
            let colorsByOperator = {
                'Claro': '#FF4560',
                'Entel': '#008FFB',
                'Bitel': '#fafe19ff',
                'Movistar': '#00E396',
                'Win': '#FEB019',
                'Claro Fibra': '#9c2f3fff',
                'WOW': '#a219feff',
                'Movistar Fibra': '#29994eff',
                'Mi Fibra': '#69beffff',
            };
