...
<input type="hidden" name="" id="typedesc" value="<?=$menuItem['TYPE_DESCRIPTION']?>">
      ...
      $.ajax({
          ...
          error: function(data){
			...
			function exportData(typeExport) {
				...
				var url = "/portalmonitoreo/public/api/getDataDownCoverage/"+tracingID+"/"+menuID+"/";
			...
			function addData() {
				...
				$.ajax({
					type:'GET',
					url:"{{ route('api.getDataMapCoverage') }}",
					data:{
			...
			function viewDataMap(dt) {
					...
					$.ajax({
						url:"{{ route('api.getDataMapCoverageCellName') }}",
						data:data,
						dataType: 'json',
						type:'GET',
						success:function (result) {
