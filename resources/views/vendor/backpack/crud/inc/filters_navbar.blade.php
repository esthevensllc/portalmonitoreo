<nav class="navbar navbar-expand-lg navbar-filters mb-0 pb-0 pt-0">
      ...
      <div class="dropdown" id="filterVisibility">
          <button class="btn btn-secondary btn-sm dropdown-toggle" type="button" id="dropdownfilterVisibility" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            Visibilidad filtros
          </button>
          <div class="dropdown-menu" aria-labelledby="dropdownfilterVisibility">
            @foreach ($crud->filters() as $filter)
              <a
                filter-name="{{ $filter->name }}"
                filter-label="{{ $filter->label }}"
                class="dropdown-item {{$filter->currentValue ? 'active' : ''}}"
                href="#"
              >{{ $filter->label }} {{ $filter->currentValue ? '(Filtered)' : '' }}</a>
            {{-- <option value=""></option> --}}
            @endforeach
          </div>
        </div>
...
<script src="{{asset('libs/URI.js/1.18.2/URI.min.js')}}"></script>
    <script>
      ...
      jQuery(document).ready(function($) {
        ...
        //ADICIONAL JS
        $('#filterVisibility .dropdown-menu').on('click', function(e) {
          e.stopPropagation();
          console.log(`${e.target.textContent} clicado!`);
        });
        $('#filterVisibility .dropdown-item').on('click', function(){
          const filterName = $(this).attr('filter-name');
          const isVisible = $(this).hasClass("active");
          const filteredText = $(`li[filter-name=${filterName}]`).attr('filter-value') == '' ? '' : '(Filtered)';
          if(isVisible){
            $(this).removeClass('active')
            $('li[filter-name='+filterName+']').hide();
          }else{
            $(this).addClass('active');
            $('li[filter-name='+filterName+']').show();
          }
          $(this).text($(this).attr('filter-label')+' '+filteredText);
